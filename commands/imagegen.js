...
module.exports = async (client, args) => {
  ...
  client.on('messageCreate', async message => {
    if (message.content.startsWith(`${prefix}text2img`)) {
       const args = message.content.slice(prefix.length).trim().split(/ +/g);
      let text2 = args.join(" ");
      if (text2.slice('text2img'.length)) {
        const { Configuration, OpenAIApi } = require("openai");
        const configuration = new Configuration({
  apiKey: process.env.OPENAI_API_KEY,// paste your api key here
});
        const openai = new OpenAIApi(configuration);
        const response = await openai.createImage({
  prompt: text2.slice('text2img'.length),
  n: 1,
  size: "1024x1024",
});
image_url = response.data.data[0].url;
        ...
        message.reply(`Where's the text? What is this? This chicanery? What a sick joke!`)
