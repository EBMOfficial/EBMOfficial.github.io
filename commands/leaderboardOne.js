...
const GlobalLeaderboard = require('../schemas/globalleaderboard.js')
...
client.on('messageReactionAdd', async (reaction, user) => {
 ...
 // Destructure the guild property from the member object
  ...
  if (reaction.emoji.name === "🔼") {
    ...
    await GlobalLeaderboard.findOneAndUpdate({ userid: reaction.message.author.id, Global: "1" }, { $inc: { points: 1 } }, { upsert: true , new: true , setDefaultsOnInsert: true })
 ...
 await lpeconomy.findOneAndUpdate({ userid: reaction.message.author.id, guildid: reaction.message.guild.id }, { $inc: { PetrolCan: 1 } }, { upsert: true , new: true , setDefaultsOnInsert: true })
    ...
    await LeaderboardSequence.findOneAndUpdate({ userid: user.id, guildID: reaction.message.guild.id }, { $inc: { points: -2 } }, { upsert: true , new: true , setDefaultsOnInsert: true })
      await GlobalLeaderboard.findOneAndUpdate({ userid: reaction.message.author.id, Global: "1" }, { $inc: { points: 2, awards: 1 } }, { upsert: true , new: true , setDefaultsOnInsert: true })
      await GlobalLeaderboard.findOneAndUpdate({ userid: user.id, Global: "1" }, { $inc: { points: -2 } }, { upsert: true , new: true , setDefaultsOnInsert: true })
    ...
    await LeaderboardSequence.findOneAndUpdate({ userid: user.id, guildID: reaction.message.guild.id }, { $inc: { points: -3 } }, { upsert: true , new: true , setDefaultsOnInsert: true })
      await GlobalLeaderboard.findOneAndUpdate({ userid: reaction.message.author.id, Global: "1" }, { $inc: { points: 3, awards: 1 } }, { upsert: true , new: true , setDefaultsOnInsert: true })
      await GlobalLeaderboard.findOneAndUpdate({ userid: user.id, Global: "1" }, { $inc: { points: -3 } }, { upsert: true , new: true , setDefaultsOnInsert: true })
    }
   ...
   if (PointAmount) {
    ...
    await LeaderboardSequence.findOneAndUpdate({ userid: user.id, guildID: reaction.message.guild.id }, { $inc: { points: -5 } }, { upsert: true , new: true , setDefaultsOnInsert: true })
      await GlobalLeaderboard.findOneAndUpdate({ userid: reaction.message.author.id, Global: "1" }, { $inc: { points: 5, awards: 1 } }, { upsert: true , new: true , setDefaultsOnInsert: true })
      await GlobalLeaderboard.findOneAndUpdate({ userid: user.id, Global: "1" }, { $inc: { points: -5 } }, { upsert: true , new: true , setDefaultsOnInsert: true })
    }
    ...
    await LeaderboardSequence.findOneAndUpdate({ userid: user.id, guildID: reaction.message.guild.id }, { $inc: { points: -10 } }, { upsert: true , new: true , setDefaultsOnInsert: true })
      await GlobalLeaderboard.findOneAndUpdate({ userid: reaction.message.author.id, Global: "1" }, { $inc: { points: 10, awards: 1 } }, { upsert: true , new: true , setDefaultsOnInsert: true })
      await GlobalLeaderboard.findOneAndUpdate({ userid: user.id, Global: "1" }, { $inc: { points: -10 } }, { upsert: true , new: true , setDefaultsOnInsert: true })
    }
    ...
    await LeaderboardSequence.findOneAndUpdate({ userid: user.id, guildID: reaction.message.guild.id }, { $inc: { points: -20 } }, { upsert: true , new: true , setDefaultsOnInsert: true })
      await GlobalLeaderboard.findOneAndUpdate({ userid: reaction.message.author.id, Global: "1" }, { $inc: { points: 20, awards: 1 } }, { upsert: true , new: true , setDefaultsOnInsert: true })
      await GlobalLeaderboard.findOneAndUpdate({ userid: user.id, Global: "1" }, { $inc: { points: -20 } }, { upsert: true , new: true , setDefaultsOnInsert: true })
    }
  }
...
client.on('messageReactionRemove', async (reaction, user) => {
 ...
 // Destructure the guild property from the member object
    ...
    const args = message.content.slice(prefix.length).split(/ +/);
    const command = args.shift().toLowerCase();
    if (command === 'globalleaderboard') {
      
     const Users = await GlobalLeaderboard.find({Global: "1"}).sort({ points: -1 })
      const embedArray = []
      for (var i = 0; i < Users.length % 10 + 100; i++) {
      const leaderboard = new Discord.MessageEmbed()
      .setTitle(`Here is the global points | awards leaderboard!`) 
      .setColor("RANDOM")
      .setThumbnail("https://pbs.twimg.com/media/D7ShRPYXoAA-XXB.jpg")
